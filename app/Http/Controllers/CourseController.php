...
public function referCandidate() {
		...
		if(Auth::user()->nUsrRoleID == 1006){
			$activeContracts = DB::table('tbltakinstituteaffiliation')
                        ...
                        ->where('tbltakinstituteaffiliation.nTrainerid', '=',Auth::user()->ausrid)
                        ->get();
			if(count($activeContracts) == 0){
				$response['msg'] = "Kindly subscribe to one of subscription plans to Refer students. Thank you.";
		...
		$data['role'] = 1004;
		// $data['password'] = str_random(8);
		$data['password'] = 'buddhijeevi';
		$response = app('App\Http\Controllers\AccountController')->userRegistration($data, Auth::user()->ausrid);
	...
	public function getCourses( $location = array(), $keyword='', $id_category='', $sector_id='', $batchid = '', $ausrid="") {
            //$batch =  new Course; 
...
//                    $batch = $batch->orWhere('tblcoursecataloguemaster.tCoursesubtitle','LIKE', '%'.$keyword.'%');
            ...
            if( $ausrid != '' ){ 
                    $batch = $batch->where('tblusermaster.ausrid', $ausrid);
            }
...
//                    );
	...
	public function studentRegistation() {
		//if(Auth::user()->nUsrRoleID == 1005 || Auth::user()->nUsrRoleID == 1006){
			return view('student-registration');
		//}else{
			//echo 'You do not have permission to access this page';die;
		//}
		}
	public function addStudentRegistation() {
		$data = Request::all();  
		$row = DB::table('tblstudentregistation')->where('tEmail', $data["email"])->get();
		$response = array();
		if( count($row) == 0 ){
			$row1 = DB::table('tblstudentregistation')->where('nAatharno', $data["aadhar"])->get();
			if( count($row1) == 0 ){
				$row2 = DB::table('tblstudentregistation')->where('nCandidateMobile', $data["mblnumber"])->get();
				if( count($row2) == 0 ){
					
					$datetime1 = date_create($data['dob']);
					...
					if( ($age >= 18) && ($age < 28)  ){
						...
						$insert_data = array();
						$insert_data['tCandidateName'] = $data['candidatename'];
						$insert_data['tFatherName'] = $data['fathername'];
						$insert_data['tMotherName'] = $data['mothername'];
						$insert_data['dob'] = $data['dob'];
						$insert_data['tGender'] = $data['gender'];
						$insert_data['nCandidateMobile'] = $data['mblnumber'];
						$insert_data['tEmail'] = $data['email'];
						$insert_data['nGuardianContact'] = $data['guardiannum'];
						$insert_data['nAatharno'] = $data['aadhar'];
						$insert_data['tCurrentLocation'] = $data['location'];
						$insert_data['tPortalAddress'] = $data['postaladdress'];
						$insert_data['tCity'] = $data['city'];
						$insert_data['nPincode'] = $data['pincode'];
						$insert_data['tState'] = $data['state'];
						$insert_data['tGoogleLocation'] = $data['googlelocation'];
						$insert_data['nOtherMblNumber'] = $data['number'];
						$insert_data['nMonthlyIncome'] = $data['income'];
						$insert_data['tDrivingLicense'] = $data['driving_license'];
						$insert_data['nDLNumber'] = $data['provide'];
						$insert_data['tAnyDisability'] = $data['disability'];
						$insert_data['tStudentDoingAtPresent'] = $data['doing'];
						$insert_data['tWillingnessToTravel'] = $data['travel'];
						$insert_data['tWorkExperience'] = $data['work_experience'];
						$insert_data['tFamilyMembers'] = $data['familydetails'];
						$insert_data['tHobbies'] = $data['hobbies'];
						$insert_data['tMigrateToCities'] = $data['intersred'];
						$insert_data['tScholarshipReason'] = $data['reason'];					
						$insert_data['n10thYear'] = $data['year_10th'];					
						$insert_data['t10thCompleted'] = $data['completed_10th'];					
						$insert_data['t10thCouldNotPoss'] = $data['could_not_pass_10th'];					
						$insert_data['t10thInstitute'] = $data['institute_10th'];	
						
						$insert_data['n12thYear'] = (!empty($data['year_12th'])?$data['year_12th']:"");					
						$insert_data['t12thCompleted'] = $data['completed_12th'];					
						$insert_data['t12thCouldNotPoss'] = $data['could_not_pass_12th'];					
						$insert_data['t12thInstitute'] = $data['institute_12th'];					
						$insert_data['nOtherQYear'] = $data['year_other'];					
						$insert_data['tOtherQCompleted'] = $data['completed_other'];					
						$insert_data['tOtherQCouldNotPoss'] = $data['could_not_pass_other'];					
						$insert_data['tOtherQInstitute'] = $data['institute_other'];					
						$insert_data['nCreatedBy'] = Auth::user()->ausrid;					
						$insert_data['dCreatedAt'] = date("Y-m-d H:i:s");					
						$result = DB::table('tblstudentregistation')->insert($insert_data);
						if( $result ){
							$response['status']='success';	
						}else{
							$response['status']='failed';
						...
						$response['status']='failed';
						$response['msg']='Candidate age should be greater than 18 Years and Less than 28 Years';
					}
				...
				$response['status']='failed';
				$response['msg']='Aadhar number already exist.';
			}	
