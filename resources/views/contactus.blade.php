            ...
            <form class="form-horizontal" name="RegForm1" id="FeedBackForm" method="post" action="/send-feedback-mail">
               ...
               <input name="_token" type="hidden" value="{!! csrf_token() !!}" />
                <div class="form-group">
                  ...
                  <div class="col-sm-5">
                      <input type="text" class="form-control" id="inputname" value="{{ old('name') }}" name="name" placeholder="First Name">
                  </div>
				  <div class="col-sm-5">
                      <input type="text" class="form-control" id="inputname" value="{{ old('lname') }}" name="lname" placeholder="Last Name">
                  </div>
                </div>
				 <?php
			
                    $instituteType = DB::table('tblinstitutetype')->orderBy('aInstituteRoleMasterID','asc')->get();
                ?>
				<div class="form-group">
                  <label for="instituteType" class="col-sm-2 control-label">I Am</label>
                  <div class="col-sm-10">
                    <select name="instituteType" id="instituteType" class="form-control">
                        <option value="">Select Option</option>
                           @foreach($instituteType as $request)
                        <option value="{{$request->aInstituteRoleMasterID}}" >{{$request->tInstituteType}}</option>
                        @endforeach                     
                    </select>
                  </div>
                </div>
				<div class="form-group">
                  ...
                  <div class="col-sm-10">
                      <input type="text" class="form-control" id="orgname" value="{{ old('orgname') }}" name="orgname">
                  </div>
				  </div>
                <div class="form-group">
